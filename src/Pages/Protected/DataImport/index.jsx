    ...
    reader.onload = () => {
      ...
      try {
        ...
        const transactionRegex =
          /<STMTTRN>[\s\S]*?<DTPOSTED>([\d]+)[\s\S]*?<TRNAMT>([-\d.]+)[\s\S]*?<NAME>(.*?)<\/NAME>[\s\S]*?<MEMO>(.*?)<\/MEMO>/g;
      ...
      if (updatedTransactions[i].amount > 0) {
        updatedData = {
          ...
          datadePagamento: new Date(
            updatedTransactions[i].date.split("/").reverse().join("-")
          ),
      ...
      if (updatedTransactions[i].flagUpdated) {
        ...
        try {
          if (!updatedTransactions[i].id) {
            ...
            await updateFinancialMovementsById(
              updatedTransactions[i].id,
              updatedData
            );
          }
              ...
              <tr key={index}>
                ...
                <td>
                  <FieldSelect
                    label=""
                    onChange={(e) =>
                      handleChangeTipoDocumento(index, e.target.value)
                    }
